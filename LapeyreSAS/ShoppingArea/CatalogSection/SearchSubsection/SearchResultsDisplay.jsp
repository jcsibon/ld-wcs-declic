...
<c:set var="searchTerm" value="${fn:escapeXml(WCParam.searchTerm)}" />
<c:if test="${!empty WCParam.manufacturer}" >
	<c:set var="searchTerm" value="${fn:escapeXml(WCParam.manufacturer)}" />
</c:if>
...
<html xmlns:wairole="http://www.w3.org/2005/01/wai-rdf/GUIRoleTaxonomy#"
	...
	<head>
		...
		<title><fmt:message bundle="${storeText}" key="TITLE_SEARCH_RESULTS"><fmt:param value="${searchTerm}" /></fmt:message></title>
...
<c:set var="onlyResultsForContent" value="0" />
		
<%@ include file="/Widgets-lapeyre/Common/SearchSetup.jspf" %>

<c:remove var="onlyResultsForContent" />		
