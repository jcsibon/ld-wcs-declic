...
/*  custom Lapeyre */
,wc.service.declare({
	id: "AjaxMercanetPreparePayment",
	actionId: " AjaxMercanetPreparePayment",
	url: " AjaxMercanetPreparePayment",
	formId: "",

	successHandler: function(serviceResponse) {
		cursor_clear();
		var paymentForm = dojo.byId("sipsForm");
		var varTitle = $('<textarea />').html(serviceResponse.mercanetForm).text();
		$("#sipsForm").html(varTitle);
	},
    failureHandler: function(serviceResponse) {
    	if (serviceResponse.errorMessage) {
    		$('#errorMessage').html(serviceResponse.errorMessage);
    	}
    } 
})
;
