...
wc.render.declareRefreshController({
	...
	postRefreshHandler: function(widget) {
		...
		// Bug Mantis 394: Ajouter le listener pour l'�v�nement onComputeTask_Event de StockAvailability avant selectDefaultSwatch
		dojo.query('script[id^="SocialNetworksScript_"]').forEach(function(node, index, nodelist){
			dojo.eval(node.innerHTML);
		});
		
		dojo.query('script[id="WCQuickInfoItemAvaibilityScripts"]').forEach(function(node, index, nodelist){
			dojo.eval(node.innerHTML);
		  });

		dojo.query('script[id="WCQuickInfoItemAvaibilityScripts_SetDefaultHandler"]').forEach(function(node, index, nodelist){
			dojo.eval(node.innerHTML);
		});
		//
		...
		//coche si le produit est d�j� dans le comparateur
		var comp = dojo.byId('comparator_'+this.currentRCProperties.productId+'_quickView');
		if(comp!=null){
			var compScript = comp.innerHTML;
			eval(compScript);
		}
		cursor_clear();
	} 
...
wc.render.declareRefreshController({
	id:"itemAvailabilityRefreshAreaController",
	renderContext: wc.render.getContextById("ItemAvailablity_Context"),
	url: "/ItemAvailabilityView",
	formId: "",
	renderContextChangedHandler: function(message, widget) {
		widget.refresh(this.renderContext.properties);
	},
	postRefreshHandler: function(widget) {
		dojo.query('script[id^="itemAvaibilityScripts"]').forEach(function(node, index, nodelist){
			dojo.eval(node.innerHTML);
		  });
		
		if(stockAvailability) {
			$(stockAvailability).trigger("onComputeTask_Event", {productId : this.renderContext.properties.catalogEntryID});
		}
	}
}),

wc.render.declareRefreshController({
	id:"WCQuickInfoItemAvailabilityRefreshAreaController",
	renderContext: wc.render.getContextById("WCQuickInfoItemAvailability_Context"),
	url : "/WCQuickInfoItemAvailabilityView",
	formId: "",
	renderContextChangedHandler: function(message, widget) {
		widget.refresh(this.renderContext.properties);
	},
	postRefreshHandler: function(widget) {
		...
		var itemAvailabilityInPhysicalStoreForQuickInfo = new ItemAvailabilityInPhysicalStoresWidget();
...
//		  });
		...
		if(stockAvailability_Popin) {
			$(stockAvailability_Popin).trigger("onComputeTask_Event", {productId : this.renderContext.properties.catalogEntryID});
		}
	}
}),

wc.render.declareRefreshController({
	id:"itemAvailabilityRefreshAreaInShopCartController",
	renderContext: wc.render.getContextById("ItemAvailablity_Context"),
	url : "/ItemAvailabilityView",
	formId: "",
	renderContextChangedHandler: function(message, widget) {
...
wc.render.declareRefreshController({
	id:"ShopsInfoController",
	renderContext: wc.render.getContextById("ShopsInfo_Context"),
	url : "",
	formId: "",
	renderContextChangedHandler: function(message, widget) {
		widget.refresh(this.renderContext.properties);
...
wc.render.declareRefreshController({
	id:"itemAvailabilityRefreshAreaInTunnelLivraisonController",
	renderContext: wc.render.getContextById("TunnelLivraisonItemAvailablity_Context"),
	url : "/ItemAvailabilityView",
	formId: "",
	renderContextChangedHandler: function(message, widget) {
		widget.refresh(this.renderContext.properties);
	...
	postRefreshHandler: function(widget) {
		dojo.query('script[id^="tunnelLivraisonItemAvaibilityScripts"]').forEach(function(node, index, nodelist){
			dojo.eval(node.innerHTML);
		  });
	}
}),
...
wc.render.declareRefreshController({
	id:"shopperActionsRefreshAreaController",
	renderContext : wc.render.getContextById("ShopperAction_Context"),
	url: "",
	formId: "",
	renderContextChangedHandler: function(message, widget) {
		widget.refresh(this.renderContext.properties);
	...
	postRefreshHandler: function(widget) {
		productDisplayJS.updateSurface(this.renderContext.properties.catalogEntryID,this.renderContext.properties.productId);
		$('#minishopcart_button_total_mobile').text($('#minishopcart_total_mobile').text());
	}
}),

wc.render.declareRefreshController({
	id:"WCQuickInfoShopperActionsRefreshAreaController",
	renderContext : wc.render.getContextById("WCQuickInfoShopperAction_Context"),
	url: "",
	formId: "",
	renderContextChangedHandler: function(message, widget) {
		widget.refresh(this.renderContext.properties);
...
wc.render.declareRefreshController({
	id: "OrderSummaryController",
	renderContext: wc.render.getContextById("OrderSummaryContext"),
	url: "",
	formId: "",
	renderContextChangedHandler: function(message, widget) {
		widget.refresh(this.renderContext.properties);
	...
	postRefreshHandler: function(widget) {
		dojo.query('script[id="TagManagerRefresh"]').forEach(function(node, index, nodelist){
			dojo.eval(node.innerHTML);
		  });
	}
})
