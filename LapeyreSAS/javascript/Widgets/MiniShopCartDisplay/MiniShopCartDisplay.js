 ...
 */
dojo.require('lapeyre.widget.Dialog');
...
function toggleMiniShopCartDropDownEvent(event,relativeId,contentId,contentType){
	...
	if(event.keyCode == dojo.keys.DOWN_ARROW || event.keyCode == dojo.keys.ENTER){
		toggleMiniShopCartDropDown(event,relativeId,contentId,contentType);
		dojo.stopEvent(event);
	}
}
...
function toggleMiniShopCartDropDown(event,relativeId,contentId,contentType){
	dojo.stopEvent(event);
...
//	var x1 = c.x + 60;
	...
	*/
	...
	var bodyPositionLeft = $('body').offset().left;
	var dialogWidth = $('#' + contentId +' #cartDropdown').width();
	dlgX = (bodyWidth - dialogWidth) + bodyPositionLeft;
	if(dialog && toRepositionne){
			dialog.y = y1;
			dialog.x = dlgX;
	}
	...
	var hide = function() {
		...
		deleteWidgetAssociationInPopin();
	};
	
	//supprimer les attributs data-dojo-type="wc/widget/WCCarousel" data-dojo-props="columnCountByWidth:${columnCountByWidth}" pour �viter d'�tre repars� par dojo
	$( "#MiniShopCartProductAdded .merchandisingAssociationWidget" ).each(function( index ) {
		  $(this).removeAttr('data-dojo-type');
		  $(this).removeAttr('data-dojo-props');
	});
	...
	if(!dialog){
		...
		if (dialogTitleElement != null) {
			if(toRepositionne) {
				dialog = new lapeyre.widget.Dialog({
					...
					_position: function() {
						var body = $('body');
						var header = $('#headerRow1');
						var headerMobile = $('#headerMobile');
						if(viewport().width >= desktopBreakpoint){
							var left = Math.floor($('#header .minishopcart_total').offset().left - $('#widget_minishopcart_popup #cartDropdown').outerWidth() + 20);
							var top  = Math.floor(header.position().top + header.height());
						}else{
							var left = Math.floor($('#headerMobile-cart .minishopcart_total').offset().left - $('#widget_minishopcart_popup #cartDropdown').outerWidth() + 20);
							var top  = Math.floor(headerMobile.position().top + headerMobile.height());
						}
						
						$(this.domNode).css('position', 'fixed');
						$(this.domNode).css('left', left + 'px');
						$(this.domNode).css('top', top + 'px');
					}
				}, pane);
				...
				dialog = new lapeyre.widget.Dialog({title:dialogTitleElement.innerHTML, onShow:show, onHide:hide},pane);
			}
			...
			require(["dijit/registry"], function(registry){
				var mywidget = registry.byId("catalogEntryRecommendationWidgetT2SAddToCart");
				
				if(mywidget != undefined) {
			    	 mywidget.destroyRecursive(true);
			     }
			});

			if(toRepositionne) {
				dialog = new lapeyre.widget.Dialog({relatedSource: relativeId, x:x1, y:y1, onShow:show, onHide:hide},pane);
			} else {
				dialog = new lapeyre.widget.Dialog({onShow:show, onHide:hide},pane);
			}
		}
		dialog.x=dlgX;
	...
	if(!dialog.displayStatus && (status == null || !status)){
		...
		dialog.autoClose = true; // Do not close the dialog when it loses focus. Use the Close button.
		if(contentType == 'orderItemsList' || contentType == 'orderItemsListAutoClose'){
			...
			if(viewport().width < desktopBreakpoint) {
				$("#MiniShopCartProductAddedNotification").fadeIn();
			} else {
		 ...
		 if(document.getElementById("currentOrderQuantity") != null && document.getElementById("currentOrderAmount") != null
			...
			&& document.getElementById("currentOrderLanguage") != null) {
				...
				if(cartOrderId != ""){
					...
					dojo.publish("/MiniShopCartDisplayRefresh", [{ total:cartQuantity}]);
					
				} else {
					setCookie("WC_CartOrderId_"+WCParamJS.storeId, "", {path:'/', domain:cookieDomain});
					dojo.publish("/MiniShopCartDisplayRefresh", [{ total: 0}]);
				}			      
		 }
}
...
//			var items = document.getElementById("minishopcart_total");
			var itemsTotals = document.getElementsByClassName("minishopcart_total");
...
function handleMiniCartHover() {
    var miniCartDesktopResults = dojo.query('#header #widget_minishopcart');
    if (miniCartDesktopResults.length > 0) {
            var miniCartDesktop = miniCartDesktopResults[0];
            
            var miniCartHoverHandle = null;
		dojo.connect(document.getElementById("widget_minishopcart"), "onmouseover", function() {
            ...
            dojo.connect(miniCartDesktop, "onmouseover", function() {   
            	miniCartHoverHandle = setTimeout(function(){showMiniShopCartDropDown('widget_minishopcart','quick_cart_container','orderItemsListAutoClose')}, 300);
                });
            dojo.connect(miniCartDesktop, "onmouseout", function() {
                    clearTimeout(miniCartHoverHandle);
            });        
    }
}
...
function deleteWidgetAssociationInPopin() {
	dojo.query('#MiniShopCartProductAdded .merchandisingAssociationWidget', document).forEach(function(tag) {		
