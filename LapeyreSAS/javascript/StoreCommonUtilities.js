...
/** This varible indicates whether a mobile device is used or not */
var mobileDevice = null;

var usingProgressBars = {};
 ...
 * @param {Integer} timeout       Number of milliseconds of timeout before showing the progress bar
 */
function cursor_wait(checkForOpera, timeout) {
	var showPopup = true;	
	...
	if(showPopup){
		...
		setTimeout(showProgressBar, timeout || 500 );
	}
}
...
//		} else {
		...
		//ECOCEA: positionner syst�matiquement le loader au centre de l'�cran
...
//		}
		...
		//Make sure the progress bar dialog goes away after 20 seconds
		//and does not hang if server calls does not return
		//Assuming the longest server calls return before 20 seconds
		setTimeout("cursor_clear()", 20000);
	}
}

/**
 * hide the progress bar pop-up
 */
function hideProgressBar(id) {
	if(id != null && trim(id).length > 0) {
		delete usingProgressBars[id];
	}
	
	if(sizeOfObject(usingProgressBars) > 0) {
		return;
	}
...
function redirectToSignOn(forcedUrl,returnPage) {
	...
	document.location.href = "/LogonForm?myAcctMain=1&storeId="
			...
			+ "&langId=" + WCParamJS.langId + "&returnPage="+ returnPage +"&URL=" + encodeURIComponent(currentURL);
}
...
function isMobileDevice() {
    if(mobileDevice == null && navigator != null && navigator.userAgent != null) {
...
function outlineSpots(){
	...
	for (var i = 0; i < all.length; i++) {
		...
		if(dojo.hasClass(currEl,"emptyESpot")){
			...
			//ECOCEA : this line resize ESpot div wich display content  
...
/**
 * Compare deux dates repr�sent�es sous le format 'dd/MM/yyyy' 
 * @returns true date1 is later than date2, false if not.
 * @param string sDate1 (ex : dd/MM/yyyy ..) 
 * @param string sDate2 (ex : dd/MM/yyyy ..)
 */
function isLater(sDate1, sDate2) {
	return parseDate(sDate1) > parseDate(sDate2);
}

/**
 * Calcule la diff�rence en jour entre date et now
 * @param string sDate (ex : dd/MM/yyyy...)
 * @returns
 */
function differenceFromDateToNow(sDate) {
	var result = null;
	require(["dojo/date"], function(date){
		var now = new Date();
		    now = new Date(now.getFullYear(), now.getMonth(), now.getDate()); // hh:mm:ss = 00:00:00
		var dateDispo = parseDate(sDate)
		result =  date.difference(now, dateDispo, 'day');
	});
	return result;
}
...
/**
 * 
 * @param sDate - une cha�ne de caract�res repr�sentant une date au format 'dd/MM/yyyy' 
 * @returns
 */
function parseDate(sDate) {
	var parsedDate;
	require(["dojo/date/locale"], function(locale){
		parsedDate = locale.parse(sDate, {
			selector: "date",
...
/**
 * @param dDate - un objet date
 * @returns la date au format 'dd/MM/yyyy'
 */
function formatDate(dDate) {
	var formattedDate;
	require(["dojo/date/locale"], function(locale) {
		formattedDate = locale.format(dDate, {
			selector: "date",
			datePattern: "dd/MM/yyyy"
		});
	})
...
function parseCMSDate(sDate) {
	//yyyyMMdd
	if(sDate != null && trim(sDate).length == 8) {
		var year = sDate.substring(0,4); year = parseInt(year);
		var month = parseInt(sDate.substring(4,6)) - 1;
...
function convertToCMSDate(date) {
	if(date != null) {
...
//IE8 Object.keys function does not exists.
if (!Object.keys) {
...
function checkNumericInput(e) {
    ...
    if ($.inArray(e.keyCode, [8, 9, 27, 13, 110, 188, 190]) !== -1 ||
		// Allow: keyboard number keys
		(e.charCode >=48 && e.charCode <=57) ||
    	// Allow: Ctrl+A
        (e.charCode == 65 && e.ctrlKey === true) ||
        // Allow: Ctrl+C
        (e.charCode == 67 && e.ctrlKey === true) ||
        // Allow: Ctrl+X
        (e.charCode == 88 && e.ctrlKey === true) ||
...
//Quantity controls with allowed dot
function checkNumericInputWithAllowedDot(e) {
    // Allow: backspace, delete, tab, escape, enter, comma, period and dot
    if ($.inArray(e.which, [44, 46, 8, 9, 27, 13, 110, 188, 190]) !== -1 ||
		// Allow: keyboard number keys
		(e.which >=48 && e.charCode <=57) ||
    	// Allow: Ctrl+A
