...
<ul id="departmentsMenu" role="menu" data-parent="header" aria-labelledby="departmentsButton">
	<c:forEach var="department" items="${categoryHierarchy.catalogGroupView}">
		<li>
			<a id="departmentButton_${department.uniqueID}" href="#" class="departmentButton" role="menuitem" aria-haspopup="true" data-toggle="departmentMenu_${department.uniqueID}"><span><c:out value="${department.name}"/></span></a>
			<div id="departmentMenu_${department.uniqueID}" class="departmentMenu" role="menu" data-parent="departmentsMenu" data-id="${department.uniqueID}" aria-label="${department.name}" lazyLoad="${lazyLoad}">
				...
				<c:if test="${!empty department.catalogGroupView}">
					<ul class="categoryList">
							...
							<li>
								...
								<c:if test="${!empty category.catalogGroupView}">
									<ul class="subcategoryList">
										...
										<c:if test="${fn:length(category.catalogGroupView) > subcategoryLimit}">
											<li class="more">
												<c:set var="key" value="${department.uniqueID}_${category.uniqueID}"/>
						...
						<c:if test="${fn:length(department.catalogGroupView) > subcategoryLimit}">
							<li class="more">
	...
	<li class="active">
		<a id="allDepartmentsButton" href="#" class="departmentButton" role="button" aria-haspopup="true" data-toggle="allDepartmentsMenu"><span><fmt:message bundle="${storeText}" key="SEARCH_ALL_DEPARTMENTS"/></span></a>
