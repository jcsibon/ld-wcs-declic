...
<c:choose>
	<c:when test="${pageName eq 'AdvancedSearchPage' || pageName eq 'StaticSearchPage' || (!empty productId && (empty parent_category_rn && empty categoryId))}">
		<%out.flush();%>
			<c:import url = "/Widgets/com.ibm.commerce.store.widgets.BreadcrumbTrail/BreadcrumbTrailGeneric.jsp" />
		<%out.flush();%>
	</c:when>
	<c:otherwise>
		<%out.flush();%>
			<c:import url = "/Widgets/com.ibm.commerce.store.widgets.BreadcrumbTrail/BreadcrumbTrailHierarchy.jsp" />
		<%out.flush();%>
	</c:otherwise>
</c:choose>
