...
<!-- BEGIN CatalogEntryDisplay.jsp pageView= ${param.pageView} -->
...
<c:if test="${!empty param.catEntryType}">
	<c:set var="catEntryType" value="${param.catEntryType}" />
</c:if>
<c:if test="${!empty param.defaultProductSkuID}">
	<c:set var="defaultProductSkuID" value="${param.defaultProductSkuID}" />
</c:if>
...
<c:if test = "${param.custom_data ne 'true'}">
	<c:choose>
		<c:when test = "${(param.pageView == 'list' or param.pageView == 'miniList') and !isOnMobileDevice}"> <%-- Mantis 0001067 : On diminue le chargement en mobile --%>
			<c:set var="productId" value="${catEntryIdentifier}"/> <%-- ProductDescription_Data expects us to pass productId --%>
			<%@ include file="/Widgets-lapeyre/Common/ProductDescription/ProductDescription_Data.jspf" %>
		</c:when>
		<c:otherwise>
...
<div class="catentrydisplay-product">
<c:if test="${env_inPreview && !env_storePreviewLink}">
	...
	<div class="ESpotInfo">
		...
		<a id="CatalogEntryDisplay_${param.pageView}_click2edit_Product_${catalogEntryID}" class="click2edit_button"  href="javascript:void(0)" onclick="parent.callManagementCenter('<wcf:out escapeFormat="js" value="${clickToEditURL}"/>');" ><fmt:message key='Click2Edit_product' bundle="${previewText}"/></a>
	</div>
</c:if>
