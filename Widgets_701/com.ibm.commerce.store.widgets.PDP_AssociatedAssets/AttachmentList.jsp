					...
					<div class="attachment">
						...
						<c:choose>
							<c:when test="${(mimePart eq 'image') || (mimePart eq 'images')|| (mimePart eq 'application') || (mimePart eq 'applications') || ( mimePart eq 'text') 							
											...
											|| (mimePart eq 'model')}"> 
										<div class="icon">
											<a href="${http}${attachmentDetails['assetPath']}" target="_new" aria-describedby="acce_attachment_${fn:toLowerCase(attachmentsList.key)}_${status.count}" aria-label="<c:out value="${attachmentNameToDisplay}"/>" id="WC_TechnicalSpecification_Image_2_${status.count}">
